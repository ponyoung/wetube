...
export const postChangePassword = async (req, res) => {
  const {
    ...
    body: { newPassword, oldPassword, newPassowordConfirmation },
  } = req;
  const user = await User.findById(_id);
  const ok = await bcrypt.compare(oldPassword, user.password);
  if (newPassword !== newPassowordConfirmation) {
  ...
  await user.save(); // This will trigger bcrypt on User.js (pre)
